...
class ComponentInfo(object):
    def __init__(self, api_key, artifactory_url, component_path, virtual_component_path=None):
        self._api_key = api_key
        self._artifactory_url = artifactory_url.strip("/")
        self._component_path = component_path.strip("/")
        temp = component_path if virtual_component_path is None else virtual_component_path
    ...
    def api_url(self, *args):
        return self._make_api_url(self._component_path, *args)

    def virtual_api_url(self, *args):
        return self._make_api_url(self._virtual_component_path, *args)
    ...
    def _make_api_url(self, component_path, *args):
        paths = map(lambda x: x.strip("/"), unpack_args(*args))
